...
module.exports = {
    ...
    compress: (opts, callback) => {
        ...
        // ensure src and dest
        if(!opts.src) return error({message: "No source for compress!"})
        if(!opts.dest) return error({message: "No destination for compress!"})
    ...
    decompress: (opts, callback) => {
        ...
        // ensure src and dest
        if(!opts.src) return error({message: "No source for decompress!"})
        if(!opts.dest) return error({message: "No destination for decompress!"})
